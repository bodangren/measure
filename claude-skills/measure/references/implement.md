...
6.  **Final Report:** Announce completion and provide a summary.
    ...
    -   **Commit Message**: `docs(measure): Synchronize docs for track '<track_description>'`
...
2.  **Ask for User Choice:**
    ...
    > B. **Archive:** Move the track's folder to `measure/archive/` and remove it from the tracks file.
...
3.  **Handle User Response:**
    ...
    -   **Review (A)**: Announce: "Please run `/measure:review` to verify your changes. You will be able to archive or delete the track after the review."
    -   **Archive (B)**: Create `measure/archive/` if needed, move track folder, remove from **Tracks Registry**, and commit: `chore(measure): Archive track '<track_description>'`.
    -   **Delete (C)**: Warn user of irreversible action, then delete folder, remove from **Tracks Registry**, and commit: `chore(measure): Delete track '<track_description>'`.
